...
d3.csv("https://raw.githubusercontent.com/LuanDopke/files/main/US_debate-ML.csv").then(function (data) {
    var lanes = [];
    var items = [];
    data.forEach(value =>{
   ...
   var laneLength = lanes.length;
    ...
    var xAxis = d3v4.axisBottom(x)//utcFormat("%M:%S");
        .ticks(d3v4.timeMinute.every(2.5))
 ...
 //corta
    gBrush
        .transition(d3v4.transition()
            .duration(750))
        .delay(500)
        .call(brush.move, [widthw0_1 * 0.0, widthw0_1 * 0.05]);
...
d3.csv("https://raw.githubusercontent.com/LuanDopke/files/main/US_debate-ML.csv").then(function (data) {
  ...
  data = data.filter(d=> (d.speaker == 'SPEAKER_02' && d.topic != '[]'))
    .map(d => {
        let item = [];
        item.topic = d.topic;
        item.time = parseFloat(d.end) - parseFloat(d.start);
    ...
    makeLollipop(groupAndSum(data, 'topic', 'time'), "#widget3_7");
});

//-------------------------------widget4_1--------------------------------------

const marginw4_1 = {top: 10, right: 20, bottom: 30, left: 35},
...
d3.csv("https://raw.githubusercontent.com/LuanDopke/files/main/US_debate-ML.csv").then(function (data) {
    ...
    tabulate(groupAndSum(a, 'speaker', 'time'), ['speaker', 'time'], '#widget4_1');
});

//tabulate(data, ['date', 'close'], '#widget3_5');
var svgw3_4 = d3.select("#widget3_4").append("svg")
...
d3.csv("https://raw.githubusercontent.com/LuanDopke/files/main/output_header.csv").then(function (data) {
    var a = data.filter(e => {
        ...
        return emot[0].label == 'ang'
    }).map(e => e.transcription).join();
    var words = a.split(/[\s.]+/g)
        .map(w => w.replace(/^[“‘"\-—()\[\]{}]+/g, ""))
        .map(w => w.replace(/[;:.!?()\[\]{},"'’”\-—]+$/g, ""))
        .map(w => w.replace(/['’]s$/g, ""))
    ...
    // var myWords = [{ word: "Running", size: "10" }, { word: "Surfing", size: "20" }, { word: "Climbing", size: "15" }, { word: "Kiting", size: "18" }, { word: "Sailing", size: "20" }, { word: "Snowboarding", size: "14" }]
...
//-------------------------------widget3_5--------------------------------------
var svgw3_5 = d3.select("#widget3_5").append("svg")
    .attr("viewBox", `0 0 ${widthw3_3} ${heightw3_3}`)
    .attr("preserveAspectRatio", "xMinYMin meet")
    .append("g")
    .attr("transform",
